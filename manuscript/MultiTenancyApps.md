...
> ![](images/professoroctopus.png) Many times our customers want to enable allowing deployments with or without a tenant because they want to test internally first.  Creating a set of internal tenants will accomplish the same goal while keeping your deployments consistent.
...
> ![](images/professoroctopus.png) Adding variables to one or two tenants at a time isn't too terrible.  However, this doesn't scale well when trying to work with 100s of tenants.  It would be a good idea to automate the tenant setup using the API.
