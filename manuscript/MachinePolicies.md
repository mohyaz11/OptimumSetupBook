...
Using the default machine policy for less than a hundred machines is excellent.  The health checks are quick.  It starts to cause problems when you have thousands of machines, and a health check might take hours to complete.  The Octopus Server can only connect to ten machines at a time to perform the health check; it can take a while to finish up.  
...
We recommend you create several machine policies.  Some possible strategies are:
...
If you go the machine policy per environment route, we recommend changing the times between health checks.  Something substantial for dev, say once a day, while production stays at once an hour.  Dev is deployed to several times per day while production deployments happen once a week. Dev machines tend to go up and down at random, change the configuration to not fail the health check.  

![](images/machinepolicies-multiplemachinepolicies.png)

> ![](images/professoroctopus.png) Each machine policy health check is a unique task.  That allows Octopus Deploy to multi-thread the health checks.  If you are using Octopus Deploy's High Availability functionality (available with a data center license), the health checks are spread across multiple nodes.  
