...
In the age of cloud providers and disaster recovery, it is very common to see scenarios where our users want to deploy to multiple data centers.  In some cases a customer could be running a live/live scenario, where the code is running in all the data centers.  In other cases we see customers wanting to deploy to the main data center first and then deploy to a disaster recovery data center.  Another common scenario we see is a data center might the "canary" where new code is deployed and maybe 5% of all traffic is routed to that data center.  Assuming everything looks good after the deployment then the code is deployed to the other data centers.  

With some of our customers we see all those scenarios being done on a single Octopus Server.  All too often we see lots of "Production" environments and a lot of unique and custom lifecycles.
...
- Internal: These are servers hosted at our main headquarters.  All development and testing is done against these servers.  
...
- Illinois Data Center: Our disaster recovery data center.  Our cloud provider has a fast connection between these two data centers.  During the day the VMs are running, whereas on the nights and weekends they are turned off.  In the event the Texas Data Center would to go offline the load balancer would automatically redirect all traffic to Illinois.
...
First we need to make some changes to our existing infrastructure.  We have already have several servers located on-premise.  The servers for Development, Testing and Staging are shared with both internal and external applications, so we want to configure those to be used for both tenanted and non-tenanted deployments.

![](images/datacenter-internaldevserverinitial.png)

This can be done by expanding out the tenant section and selecting `Include in both tenanted and untenated deployments` and then selecting the internal tenant.
...
> <img src="images/professoroctopus.png" style="float: left;"> This is somewhat contradictory to the recommendation about environments.  Environments are used almost everywhere, lifecycles, variable scoping, step scoping, etc.  Machine display names are used much less, they are primarly used for logging during a deployment.  While it is possible to scope a variable to them, that is exceedingly rare.

![](images/datacenter-stagingdatacentermachine.png)

## Projects

Next up is project configuration.  For the purposes of this demo we went ahead and cloned the existing projects and called them `OctoFx-Database Multi Data Center`, `OctoFx-Traffic Cop Multi Data Center`, and `OctoFx-Web Multi Data Center`.  In the real world you wouldn't have two projects, one tenanted and the other untenanted.  We need to let the project know that it will be used for multi-tenant deployments.  Go to the project and then click on the settings link on the left.  Expand out the multi-tenant deployment section and select `Require a tenant for deployments`.
...
Lets take a quick step back and think about this specific project, which handles the database deployments.  What would be different between the data centers?  The database server would be different.  But other than that, everything else should be the same.  To account for these differences we are going to create a project template variable.  The nice thing about the project template variables is they also support the ability to mark the value as sensitive.

![](images/datacenter-projecttemplate.png)

We will have to set up a couple of project template variables for the WebUI project, but that process is the same.  Aside from that, that is all we really need to configure on the project side.

## Tenants

Finally we need to link the data center tenants up to the projects.  This is done by going to the tenants screen, selecting the tenant you wish to link up, and then clicking the `Connect Projects` button.
...
When we look at the database project's dashboard we can see the view has changed from the typical untentated deployment dashboard.  Make note that you cannot deploy to the Texas or Illinois Data Centers for the `Development` or `Testing` environments.
...
Now we can deploy to both the Illinois and the Texas Data Center. 
...
In this chapter we walked through how to configure a Multiple Data Center deployment using tenants.  The primary advantage to using tenants is you can keep the same lifecycle as before (Development -> Testing -> Staging -> Production), but still have the flexbility to choose which data center to deploy to.  It also provides the ability to know when specific variables are scoped to a data center which you must change.  Finally, each deployment to each data center is treated as a unique task, allowing you to deploy to both data centers at the same time.